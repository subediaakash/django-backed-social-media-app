...
export default function GroupFeedComponent({
  group,
  posts,
  isLoading,
  loadError,
    ...
    <CardComponent className="flex flex-col gap-6 p-6">
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <p className="mt-2 text-xs text-neutral-400">
              Created by {ownerDisplayName} · {group.membersCount} members
            </p>
          </div>
          {group.isOwner && (
      ...
      <section className="space-y-5">
        {isLoading ? (
          ...
          <EmptyState isMember={group.isMember} />
        ) : (
          posts.map((post) => (
            <PostCard
              key={post.id}
...
function CreatePostCard({
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!trimmed) {
      setLocalError("Your post needs some content.");
      return;
    }
  ...
  return (
    <CardComponent className="space-y-4 border border-rose-100 bg-white/90 p-5 shadow-sm shadow-rose-100">
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div>
          <textarea
            aria-label="Share something with the group"
...
function EmptyState({ isMember }: { isMember: boolean }) {
  return (
